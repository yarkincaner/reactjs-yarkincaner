...
export default function Repo(props) {
  const topicsList =
    props.topics.length !== 0 ? (
      props.topics.map((topic) => (
        <p
          key={topic.id}
          className="bg-gray-800 shadow-md rounded-lg text-center text-gray-200 p-2 text-sm"
        >
          {topic}
        </p>
      ))
  ...
  return (
    <div className="flex flex-col max-w-lg h-full border border-gray-200 rounded overflow-hidden bg-inherit self-center">
      <a target="_blank" className="group flex-grow" href={props.url}>
        ...
        <p className="text-gray-200 text-lg p-4">{props.description}</p>
      </a>
